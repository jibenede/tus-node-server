...
class TusServer extends EventEmitter {
    ...
    handle(req, res) {
        ...
        // Some environments will automatically add the following header if no content-type is specified, providing
        // no API to disallow this behavior. We remove it here to adhere to protocol validation.
        if (req.headers['content-type'] && req.headers['content-type'].toLowerCase() === 'application/x-www-form-urlencoded') {
